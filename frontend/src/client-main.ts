...
// --------------------------------------------------------------------------
async function init() {
  // 1. Cache Elements & Layout Dimensions
  try {
    ...
    console.warn('Scroll init warning:', err);
  }

  // 2. Early Event Listeners Setup (Guarantees hamburger, search, auth buttons always work)
  try {
    setupEventListeners();
  } catch (err) {
    console.warn('setupEventListeners warning:', err);
  }
  ...
  try { FeedSection.renderCategories(); } catch (e) {}
  try { FeedSection.renderSubCategories(); } catch (e) {}
  try { HeroSection.renderBreakingBanner(); } catch (e) {}
  try { HeroSection.render(); } catch (e) {}
  try { BentoSection.renderBillboardAd(); } catch (e) {}
  try { BentoSection.render(); } catch (e) {}
  try { BentoSection.renderShoppingCarousel(); } catch (e) {}
  try { BentoSection.renderMidstreamAd(); } catch (e) {}
  try { DeepTechSection.render(); } catch (e) {}
  try { FeedSection.renderFilterTags(); } catch (e) {}
  ...
  Promise.allSettled([
    ...
    try { updateCachedDimensions(); } catch (e) {}
    try { FeedSection.renderCategories(); } catch (e) {}
    try { FeedSection.renderSubCategories(); } catch (e) {}
    try { HeroSection.renderBreakingBanner(); } catch (e) {}
    try { HeroSection.render(); } catch (e) {}
  ...
  // 7. Utilities & Routing
  try { GoogleTranslateService.init(); } catch (e) {}
  try { PWAUtils.setupPWAInstallPrompt(); } catch (e) {}
  try { CookieConsent.init(); } catch (e) {}
  try { updateFooterLabels(); } catch (e) {}
  try { updateFilterLabels(); } catch (e) {}
  try { SeoService.setHomeSEO(); } catch (e) {}
...
function setupEventListeners() {
  ...
  document.getElementById('m-user-auth-btn')?.addEventListener('click', (e) => {
    e.preventDefault();
    window.dispatchEvent(new CustomEvent('open-auth-modal'));
  });

  // Dropdown Items Event Wiring
  document.getElementById('dropdown-auth-action-btn')?.addEventListener('click', (e) => {
    e.preventDefault();
    closeUserAvatarDropdown();
    window.dispatchEvent(new CustomEvent('open-auth-modal'));
  });

  document.getElementById('dropdown-bookmarks-btn')?.addEventListener('click', (e) => {
    e.preventDefault();
    closeUserAvatarDropdown();
    window.dispatchEvent(new CustomEvent('open-bookmarks-drawer'));
  });

  document.getElementById('dropdown-admin-cms-btn')?.addEventListener('click', () => {
    closeUserAvatarDropdown();
    window.location.href = '/admin';
  });

  // Global Search Triggers Wiring - Toggles the header Search Overlay Bar
  const searchTriggers = document.querySelectorAll('#btn-search-toggle, #btn-search-trigger, .btn-search-toggle, .btn-search-trigger');
  ...
  // Search Overlay Bar Controller (Expandable Masthead Search Bar)
  ...
  const toggleSearchOverlay = () => {
    if (searchOverlayBar) {
      const isHidden = searchOverlayBar.style.display === 'none' || !searchOverlayBar.style.display;
      if (isHidden) {
        openSearchOverlay();
      } else {
        closeSearchOverlay();
  ...
  window.addEventListener('open-search-bar', () => openSearchOverlay());
  window.addEventListener('close-search-bar', () => closeSearchOverlay());
  window.addEventListener('toggle-search-bar', () => toggleSearchOverlay());
  ...
  if (footerDirectoryGrid) {
    if (footerDirectoryGrid.children.length === 0) {
    ...
    footerDirectoryGrid.querySelectorAll('.footer-dir-cat-title').forEach(hdr => {
      hdr.addEventListener('click', (e) => {
        ...
        if (cat && (window.location.pathname === '/' || window.location.pathname === '')) {
          e.preventDefault();
    ...
    footerDirectoryGrid.querySelectorAll('.footer-dir-subcat-link').forEach(link => {
      link.addEventListener('click', (e) => {
        ...
        if (cat && sub && (window.location.pathname === '/' || window.location.pathname === '')) {
          e.preventDefault();
  ...
  // --------------------------------------------------------------------------
  // Hamburger Menu & Structured Directory Controller
  // --------------------------------------------------------------------------
  ...
  const closeCategoryDrawer = () => {
    ...
    window.dispatchEvent(new CustomEvent('close-category-drawer'));
  };
  ...
  if (drawerCategoriesTree) {
    // Only populate via JS if SSR didn't already render categories
    if (drawerCategoriesTree.children.length === 0) {
      const categories14 = CATEGORIES.filter(c => c.id !== 'all');
      drawerCategoriesTree.innerHTML = categories14.map(cat => {
        const subCats = MASTER_TAXONOMY[cat.id] || [];
        return `
          <div class="directory-col-group">
            <a href="/${cat.id}" class="directory-cat-title" data-cat="${cat.id}">
              ${cat.name}
            </a>
            <div class="directory-subcats-stack">
              ${subCats.map(sub => `
                <a href="/${cat.id}?sub=${sub.slug}" class="directory-subcat-link" data-cat="${cat.id}" data-subcat="${sub.slug}">
                  ${sub.name}
                </a>
              `).join('')}
            </div>
