            ...
            <div style="display: flex; flex-direction: column; gap: 1.75rem; font-size: 0.95rem; line-height: 1.7; color: var(--text-primary);">
              ...
              <div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                  ${(() => {
                    const activeAuthors = AuthorService.getAuthors();
                    const ceo = activeAuthors.find(a => a.role?.toLowerCase().includes('ceo') || a.role?.toLowerCase().includes('direktur')) || { name: 'Rijal Umami', role: 'Direktur Utama / CEO' };
                    const pemred = activeAuthors.find(a => a.role?.toLowerCase().includes('pemimpin redaksi') && !a.role?.toLowerCase().includes('wakil'));
                    const penasihat = activeAuthors.find(a => a.division?.toLowerCase().includes('penasihat') || a.role?.toLowerCase().includes('penasihat'));
                    const members = [ceo, pemred, penasihat].filter(Boolean);

                    return members.map(m => `
                      <div style="background: var(--bg-tertiary); padding: 1rem; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                        <span style="font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; display: block;">${m!.role || 'Redaksi'}</span>
                        <strong style="display: block; font-size: 0.9rem; margin-top: 0.25rem; color: var(--text-primary);">${m!.name}</strong>
                      </div>
                    `).join('');
                  })()}
                </div>
